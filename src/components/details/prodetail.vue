      ...
      addShop(){
          ...
          if(n === 2){
              ...
              if(sessionStorage.getItem(this.id)){
                var shopStr = JSON.parse(sessionStorage.getItem(this.id));
                var tag = false;
               shopStr.crr.map((val)=> {
                  if (val.big === this.big && val.age === this.age) {
                    val.num += Number(num);
                    tag = true
                  }
                });
               //只有循环完毕才能知道是否有相同规格的东西，因为每次只能传进来一个
               if(!tag){
                 shopStr.crr.push({
                   num,
                ...
                sessionStorage.setItem(this.id,JSON.stringify(shopStr));
              }else {
                var obj ={
                  pid: this.id,
                  name:this.obj.name,
                ...
                sessionStorage.setItem(this.id,JSON.stringify(obj));
              }
              this.$router.push({
                path:'/shopcar',
              });
